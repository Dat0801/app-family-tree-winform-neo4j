...
namespace Report
{
    public class ExcelExport
    {
        ...
        public const string T_DanhSachThanhVien = "DanhSachThanhVien";
        #endregion
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
         ...
         //   string path = Global.AppPath + Constants.FOLDER_TEMPLATES + Constants.CHAR_FLASH; //+ Constants.FOLDER_EXCELS + Constants.CHAR_FLASH;
            ...
            switch (viewName)
            {
                ...
                case T_DanhSachThanhVien:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("DanhSachThanhVien.xls").ToArray();
                    break;
                #endregion
            }
            // Get stream
        ...
        public bool ExportKhoa(List<Person> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
        ...
        //    replacer.Add(V_TONGSO, "100");
            ...
            return OutSimpleReport(dataSource, replacer, "DanhSachThanhVien", isPrintPreview, ref fileName);
        }
